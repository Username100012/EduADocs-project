...
def generate_slide_presentation(params):
    """Generate Slide Presentation presentation"""
    
    prompt = _build_slide_presentation_prompt(params)
    ...
    try:
        ...
        # Create slide_presentation file
        pptx_file = _create_slide_presentation_pptx(content, params)
        ...
        return {"success": False, "error": f"Slide presentation generation failed: {str(e)}"}

def _build_slide_presentation_prompt(params):
    """Build prompt for Slide Presentation generation"""
    
    prompt = f"""
    Create a {params['num_slides']}-slide Slide Presentation presentation outline for {params['subject']} at {params['grade_level']} level.
...
def _create_slide_presentation_pptx(content, params):
    """Create Slide Presentation file from content"""
    
    try:
        prs = Presentation()
        
        # Parse content and create slides
        slides_data = _parse_slide_presentation_content(content)
        ...
        raise Exception(f"Failed to create Slide Presentation file: {str(e)}")
...
def _parse_slide_presentation_content(content):
