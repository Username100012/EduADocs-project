...
def main():
    ...
    # Map localized label to canonical key using text matching so order changes are safe.
    doc_type_map = {}
    for option in doc_type_options:
        normalized = option.lower()
        if "lesson plan" in normalized or ("plano" in normalized and "aula" in normalized):
            doc_type_map[option] = "lesson_plan"
        elif "lecture notes" in normalized or ("notas" in normalized and "aula" in normalized):
            doc_type_map[option] = "lecture_notes"
        elif "exercise" in normalized or "exerc" in normalized:
            doc_type_map[option] = "exercise"
        elif "slide" in normalized or "apresenta" in normalized:
            doc_type_map[option] = "slide_presentation"
        elif "mind map" in normalized or "mapa mental" in normalized:
            doc_type_map[option] = "mind_map"
    doc_type_key = doc_type_map.get(doc_type, "lesson_plan")
    
    lecture_output_key = "lecture_notes"
    ...
    if doc_type_key == "lesson_plan":  # Lesson Plan
        ...
        lecture_output_options = i18n_list("lecture_notes.output_type_options")
        lecture_output = st.selectbox(
            i18n("lecture_notes.output_type_label"),
            lecture_output_options
        )

        normalized_lecture_output = (lecture_output or "").lower()
        if "slide" in normalized_lecture_output or "apresenta" in normalized_lecture_output:
            lecture_output_key = "slide_presentation"

        if lecture_output_key == "lecture_notes":
            ...
            lecture_include_examples = st.checkbox(
        ...
        mind_map_include_examples = st.checkbox(
        ...
        if is_valid:
            with st.spinner(i18n("generation.spinner_message")):
                try:
                    ...
                    effective_doc_type_key = doc_type_key
                    if doc_type_key == "lecture_notes" and lecture_output_key == "slide_presentation":
                        effective_doc_type_key = "slide_presentation"
                    ...
                    if effective_doc_type_key == "lesson_plan":
                        params.update({
                    ...
                    elif effective_doc_type_key == "lecture_notes":
                        params.update({
                            "detail_level": detail_level,
                            "format_style": format_style,
                            "include_examples": lecture_include_examples,
                            "include_references": include_references
                        })
                    elif effective_doc_type_key == "exercise":
                        params.update({
                    ...
                    elif effective_doc_type_key == "slide_presentation":
                        params.update({
                            "num_slides": num_slides,
                            "include_images": include_images,
                            "presentation_style": presentation_style
                        })
                    elif effective_doc_type_key == "mind_map":
                        params.update({
                            "main_branches": main_branches,
                            "depth_levels": depth_levels,
                            "include_examples": mind_map_include_examples,
